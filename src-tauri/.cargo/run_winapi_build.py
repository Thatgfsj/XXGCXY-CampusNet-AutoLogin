#!/usr/bin/env python3
"""Emulate the winapi build.rs to generate the correct output."""
...
# Features requested by dependent crates (from cargo metadata)
REQUESTED_FEATURES = [
    "basetsd",
    "dbghelp",
    "errhandlingapi",
...
# Build the graph
graph = {name: {"required": False, "included": False, "deps": deps, "libs": libs}
         for name, deps, libs in DATA}

# Mark required features
for feat in REQUESTED_FEATURES:
    if feat in graph:
        graph[feat]["required"] = True
        graph[feat]["included"] = True
    else:
        print(f"WARNING: Feature '{feat}' not found in DATA table")
    ...
    for name, node in graph.items():
        if node["included"]:
            for dep in node["deps"]:
                if dep in graph and not graph[dep]["included"]:
                    graph[dep]["included"] = True
                    changed = True

# Emit cfg features for ALL included features (both required and transitive)
# Cargo 1.95 precalculated optimization doesn't pass --cfg for features
# properly, so we need to emit ALL of them.
output_lines = []
for name, node in sorted(graph.items()):
    if node["included"]:
        output_lines.append(f'cargo:rustc-cfg=feature="{name}"')

# Emit library dependencies
libs = set()
for name, node in graph.items():
    if node["included"]:
...
for lib in sorted(libs):
    output_lines.append(f'cargo:rustc-link-lib=dylib={lib}')
...
OUTPUT_PATH = "C:/Users/thatg/AppData/Local/Temp/XXGCXY-CampusNet-AutoLogin/src-tauri/target/release/build/winapi-4265908cad4bf85d/output"
with open(OUTPUT_PATH, 'w') as f:
    f.write(output)
    f.write('\n')

print(f"\n---\nWritten to: {OUTPUT_PATH}")
print(f"Features emitted: {len([l for l in output_lines if l.startswith('cargo:rustc-cfg')])}")
print(f"Libraries emitted: {len(libs)}")
print(f"Libraries: {sorted(libs)}")
