...
- **v2.2.6**：**认证方法透明化、登录后状态即时刷新与检测-认证链路提速**：
  1. **日志明确实际认证方法**：登录开始日志改为中性表述，成功/失败日志注明实际路径（"Rust原生直发" / "外部脚本·PowerShell"），前端同步移除"原生直发优先"硬编码文案，用户一眼可辨本次认证由谁完成；
  2. **登录后状态即时刷新**：修复"保存并立即登录"与手动"立即登录"完成后主页仍显示"未连接"直到下一检测周期的问题——这些入口现在完成认证后立即广播 ServiceStatus（成功 Connected / 业务拒绝 Backoff / 其余 NeedsLogin）；
  3. **检测-认证链路提速**：① 双探针并发仲裁（任一 204 即在线、任一 3xx 即需登录），离线判定最坏耗时 12s→6.5s；② **探测即取参**——离线探测拿到的 302 Location 顺路解析为认证参数直接复用，正常离线→发认证路径省去最长 7.5s 的独立嗅探；③ 登录成功后下一轮探测缩短为 3 秒，尽快 204 复验确认 Connected；
  4. **主页版本号动态化**：版本徽章改为运行时读取应用真实版本（`getVersion()`，以 tauri.conf.json 为唯一事实源），以后升级不再出现界面版本与安装包不一致。
