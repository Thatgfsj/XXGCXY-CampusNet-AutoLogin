...
22. ✅ **(v2.0.7+)** 桌面端 UI 深度重构为深色极客毛玻璃风格，提供 140px 发光动态网络健康雷达仪表环、专属运营商学生名片、可折叠终端等宽日志与平滑滑动开关，保持底层 IPC 接口完全兼容
23. ✅ **(v2.0.7+)** 支持 Windows 移动热点常开守护 (Hotspot Keep-Alive)，利用原生 WinRT API 实时检测与重新唤醒，防止无设备连接自动关闭，持续共享校园网连接
24. ✅ **(v2.0.7+)** 支持字母与数字组合账号（兼容补卡后缀如 ls/lls 及前缀字母工号，输入含 @ 自动剥离），解除纯数字限制
25. ✅ **(v2.0.7+)** 开机自启动静默运行优化（注册表与 desktop 声明 --autostart，窗口初始设为不可见，开机绝不弹窗打扰用户）
